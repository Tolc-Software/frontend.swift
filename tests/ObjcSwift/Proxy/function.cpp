...
TEST_CASE("Static global function", "[function]") {
	ObjcSwift::Proxy::Function f("f", "MyNamespace::f");
	f.setAsStatic();

	auto pybindCode = f.getObjcSwift();
	CAPTURE(pybindCode);
...
TEST_CASE("Return value policy", "[function]") {
	using ReturnValue = ObjcSwift::Proxy::Function::return_value_policy;
	...
	for (auto const& policy : policies) {
		...
		auto containsString = fmt::format(
		    R"(, py::{})", ObjcSwift::returnValuePolicyToString(policy));
