...
TEST_CASE("Empty class", "[class]") {
	...
	ObjcSwift::Proxy::Class c(className, className);
...
TEST_CASE("Class with functions", "[class]") {
	...
	for (auto const& function : functions) {
		c.addFunction(ObjcSwift::Proxy::Function(function, function));
	}
...
TEST_CASE("Class with enum", "[class]") {
	...
	ObjcSwift::Proxy::Enum e(enumName, fullyQualifiedName);
	e.setScoped(false);
	e.addValue("Hi");
	c.addEnum(e);
	...
	auto expectedContains = fmt::format(
	    ...
	    fmt::arg("className", className));
	auto pybind = c.getObjcSwift(moduleName);
	CAPTURE(pybind);
	CAPTURE(expectedContains);
...
TEST_CASE("Class with constructor", "[class]") {
	...
	auto constructor = ObjcSwift::Proxy::Function(className, className);
