	...
	switch (container) {
		...
		case IR::ContainerType::UnorderedMultiSet:
			spdlog::error(
			    ...
			    ObjcSwift::Helpers::toString(container));
			break;
...
std::optional<std::string> extractInclude(IR::Type const& type) {
	if (auto container = ObjcSwift::Helpers::getContainer(type)) {
		return getInclude(container->m_container);
	} else if (ObjcSwift::Helpers::isFunctionType(type)) {
		// The type is std::function
		return "<pybind11/functional.h>";
	} else if (ObjcSwift::Helpers::isBaseType(type,
	                                          IR::BaseType::FilesystemPath)) {
		// The type is std::filesystem::path
		return "<pybind11/stl/filesystem.h>";
	} else if (ObjcSwift::Helpers::isBaseType(type, IR::BaseType::Complex)) {
...
getFirstUserDefinedRepresentation(std::vector<IR::Type> const& types) {
	if (!types.empty()) {
		if (auto userDefined = ObjcSwift::Helpers::getUserDefined(types[0])) {
...
void checkType(IR::Type const& type, ObjcSwift::Proxy::TypeInfo& info) {
	...
	while (!typesToCheck.empty()) {
		...
		if (auto container = ObjcSwift::Helpers::getContainer(current)) {
